...
import {fetchAppoinments, fetchSuccessfulAppointments } from '../../lib/fetchData';
import { EyeIcon, PencilIcon, PencilSquareIcon, TrashIcon } from "@heroicons/react/24/outline";
import Link from 'next/link';
import Pagination from '@mui/material/Pagination';


export default function index({SuccessfulAppointments}) {
    ...
    const filteredSuccessfulAppointments = useMemo(() => {
      return SuccessfulAppointments.filter((Appoinment) =>
      Appoinment.bookingrefnum.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }, [SuccessfulAppointments, searchTerm]);
  
    // Get current users
    const indexOfLastUser = currentPage * usersPerPage;
    const indexOfFirstUser = indexOfLastUser - usersPerPage;
    const AllSuccessfulAppointments = filteredSuccessfulAppointments.slice(indexOfFirstUser, indexOfLastUser);
  ...
  return (
    <div className="min-h-screen">
        <p className="flex items-center justify-center text-gray-700 text-3xl mb-16 font-bold">Successful Appointments</p>
    <div>
      <div>
        <input
          type="text"
      ...
      <TableContainer component={Paper} className="p-15 flex-1 min-w-screen mx-4">
        <Table aria-label="simple table">
          ...
          <TableBody>
            {AllSuccessfulAppointments.map((Appoinment) => (
              <TableRow key={Appoinment.id}>
                ...
                <CustomTableCell> 
                 <Link href={`/CompletedAppointments/${Appoinment.id}/edit`}>
      ...
      <Pagination
        ...
        count={Math.ceil(filteredSuccessfulAppointments.length / usersPerPage)}
        page={currentPage}
...
export async function getServerSideProps() {
    const SuccessfulAppointments = await fetchSuccessfulAppointments();

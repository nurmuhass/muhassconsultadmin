...
import {fetchAppoinments, fetchFailedRequest } from '../../lib/fetchData';
import { EyeIcon, PencilIcon, PencilSquareIcon, TrashIcon } from "@heroicons/react/24/outline";
import Link from 'next/link';
import Pagination from '@mui/material/Pagination';


export default function index({FailedRequest}) {
    ...
    const filteredFailedRequest = useMemo(() => {
      return FailedRequest.filter((Appoinment) =>
      Appoinment.bookingrefnum.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }, [FailedRequest, searchTerm]);
  
    // Get current users
    const indexOfLastUser = currentPage * usersPerPage;
    const indexOfFirstUser = indexOfLastUser - usersPerPage;
    const AllFailedRequest = filteredFailedRequest.slice(indexOfFirstUser, indexOfLastUser);
  ...
  return (
    <div className="min-h-screen">
        <p className="flex items-center justify-center text-gray-700 text-3xl mb-16 font-bold">Failed Appointments Request</p>
    <div>
      <div>
        <input
          type="text"
      ...
      <TableContainer component={Paper} className="p-15 flex-1 min-w-screen mx-4">
        <Table aria-label="simple table">
          ...
          <TableBody>
            {AllFailedRequest.map((Appoinment) => (
              <TableRow key={Appoinment.id}>
                ...
                <CustomTableCell> 
                 <Link href={`/failedRequest/${Appoinment.id}/edit`}>
      ...
      <Pagination
        ...
        count={Math.ceil(filteredFailedRequest.length / usersPerPage)}
        page={currentPage}
...
export async function getServerSideProps() {
    const FailedRequest = await fetchFailedRequest();
